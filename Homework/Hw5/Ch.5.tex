...
\end{homework}
\begin{solution}
    以下是~\ce{C6H6}分别在溶液和气相中的红外光谱图(https://doi.org/10.18434/T4D303):
    \begin{figure}[H]\centering
        \begin{subfigure}[b]{.95\linewidth}
            \centering\includegraphics[scale=0.6]{figure/Gas.png}
            \caption{Gas (70 mmHg, \ce{N2} Added, Total pressure 600 mmHg)}
        \end{subfigure}
        \begin{subfigure}[b]{.95\linewidth}
            \centering\includegraphics[scale=0.6]{figure/Sol.png}
            \caption{Solution (10\% in \ce{CCl4} for 3800-1300, 10\% in \ce{CS2} for 1300-625, and 10\% in \ce{CCl4} for 625-240 \si{\per\centi\meter}) VS. Solvent}
        \end{subfigure}
    \end{figure}
    两张谱图的主要吸收峰位置总体一致,包括在$\SI{3000}{\per\centi\meter}$附近的芳环~\ce{C-H}键伸缩振动,在$\SI{1500}{\per\centi\meter}$附近的芳环骨架~\ce{C-C}键伸缩振动等.两者的本质是相同的.\\
    \indent 在气相的谱图中,吸收峰更窄,并且出现明显的分裂,说明转动能级没有被限制,可以观察到转动-振动复合的现象.而溶液中由于溶剂分子不断碰撞导致上述精细结构消失,吸收峰表现为单峰.
\end{solution}
\begin{homework}[7.]
    回答下列问题.
    \begin{enumerate}[label=\tbf{(\alph*)}]
...
\begin{enumerate}[label=\tbf{(\alph*)}]
    ...
    于是势能曲线为
    \[V(x)=hc\tilde{D}_e(1-\e^{-ax})^2=8.591\times10^{-19}(1-\e^{-\SI{1.732e10}{\per\meter}\cdot x})^2\ \text{J}\]
    零点能为
...
\begin{solution}
    ...
    \begin{enumerate}[label=\tbf{(\alph*)}]
        \item 首先$\Pi$组态来源于$\sigma$与$\pi$电子的耦合,或两个$\pi$电子的耦合.齐次,$g$对称性要求必须是$g\to g$或$u\to u$的跃迁.因此对应于$2\sigma_g\to1\pi_g$或$1\pi_u\to2\sigma_u$.
        \item 跃迁后仍为$\Sigma$态,只能来源于两个$\pi$电子的耦合,并且$u$对称性要求跃迁前后对称性相反.于是对应于$1\pi_u\to1\pi_g$.
        \item 同样, $\Delta$态也只能来源于两个$\pi$电子的耦合,因此对应于$1\pi_u\to1\pi_g$.
        \item 与\tbf{(a)}相似,但是对称性相反,因此只能对应于$1\pi_u\to1\pi_g$.
        \item 与\tbf{(b)}相似,并且三重态也可以来源于两个$\pi$电子的耦合.于是对应于$1\pi_u\to1\pi_g$.
    \end{enumerate}
\end{solution}
\begin{homework}[12.]
    下列哪种跃迁是电偶极所允许的?
...
\end{homework}
\begin{solution}
    电偶极跃迁需要满足$\Delta S=0$, $\Delta\Lambda=0,\pm1$,且对$\Sigma$态必须保持对称性.因此允许的跃迁为\tbf{(i)}, \tbf{(ii)}和\tbf{(v)}.
\end{solution}
\begin{homework}[13.]
    根据群论,以下哪种跃迁是电偶极所允许的?
    \begin{center}
...
\begin{solution}
    ...
    必须包含全对称表示.下面分情况考虑:
    \begin{enumerate}[label=\tbf{(\alph*)}]
        \item 乙烯属于$D_{2\text{h}}$点群. $\pi$轨道的对称性为$\text{B}_{1\text{u}}$, $\pi^\ast$轨道的对称性为$\text{B}_{2\text{g}}$.在$D_{2\text{h}}$点群中$x$, $y$和$z$分别对应于$\text{B}_{3\text{u}}$, $\text{B}_{2\text{u}}$和$\text{B}_{1\text{u}}$.分别有
        \[\text{B}_{1\text{u}}\otimes\text{B}_{3\text{u}}\otimes\text{B}_{2\text{g}}=\text{B}_{2\text{g}}\otimes\text{B}_{2\text{g}}=\text{A}_{\text{g}}\]
        \[\text{B}_{1\text{u}}\otimes\text{B}_{2\text{u}}\otimes\text{B}_{2\text{g}}=\text{B}_{3\text{g}}\otimes\text{B}_{2\text{g}}=\text{B}_{1\text{g}}\nsupset\text{A}_{\text{g}}\]
        \[\text{B}_{1\text{u}}\otimes\text{B}_{1\text{u}}\otimes\text{B}_{2\text{g}}=\text{A}_{\text{g}}\otimes\text{B}_{2\text{g}}=\text{B}_{2\text{g}}\nsupset\text{A}_{\text{g}}\]
        因此是允许的.
        \item 同样地, 参与跃迁的$n$电子的对称性为$\text{B}_1$.而$\pi^\ast$轨道的对称性为$\text{B}_1$.在$D_{2\text{h}}$点群中$x$, $y$和$z$分别对应于$\text{B}_{1}$, $\text{B}_{2}$和$\text{A}_{1}$对称性.分别有
        \[\text{B}_2\otimes\text{B}_1\otimes\text{B}_1=\text{B}_2\otimes\text{A}_1=\text{B}_2\nsupset\text{A}_1\]
        \[\text{B}_2\otimes\text{B}_2\otimes\text{B}_1=\text{A}_1\otimes\text{B}_1=\text{B}_1\nsupset\text{A}_1\]
        \[\text{B}_2\otimes\text{A}_1\otimes\text{B}_1=\text{B}_2\otimes\text{A}_1=\text{B}_1\nsupset\text{A}_2\]
        因此跃迁是禁阻的.
    \end{enumerate}
\end{solution}
\begin{homework}[14.]
...
\begin{solution}
    \begin{enumerate}[label=\tbf{(\alph*)}]
        \item 只需证明对任意$i\neq j$都有$\hat{H}'_{ij}=\bra{\phi_i}\hat{H}'\ket{\phi_j}=0$即可.考虑~\ce{Ti(H2O)6^{3+}}为$O_{\text{h}}$点群的分子,则
        \[\Gamma(\phi_1)=\Gamma(\phi_2)=\text{E}_{\text{g}},\quad\Gamma(\phi_3)=\Gamma(\phi_4)=\Gamma(\phi_5)=\text{T}_{1\text{u}}\]
        而$\Gamma(\hat{H}')=\text{A}_{1\text{g}}$.考虑过$z$轴的$C_4$操作,有
        \[C_4\phi_1=\phi_1,\quad C_4\phi_2=-\phi_2,\quad C_4\phi_3=\phi_4,\quad C_4\phi_4=-\phi_3,\quad C_4\phi_5=-\phi_5\]
        由于$\hat{H}'$是厄米算符,并且$C_4\hat{H}'=\hat{H}'$,因此
        \[\bra{\phi_1}\hat{H}'\ket{\phi_2}=\bra{\phi_1}\left(\hat{H}'C_4\right)\ket{\phi_2}=\bra{\phi_1}\hat{H}'\left(C_4\ket{\phi_2}\right)=-\bra{\phi_1}\hat{H}'\ket{\phi_2}\]
        于是$\hat{H}'_{12}=\bra{\phi_1}\hat{H}'\ket{\phi_2}=0$.同样地,对于$i,j\in\{1,2,5\}$且$i\neq j$都有$\hat{H}'_{ij}=0$.\\
        同样地有
        \[\bra{\phi_1}\hat{H}'\ket{\phi_3}=\bra{\phi_1}\hat{H}'C_4\ket{\phi_3}=\bra{\phi_1}\hat{H}'\ket{\phi_4}=\bra{\phi_1}\hat{H}'C_4\ket{\phi_4}=-\bra{\phi_1}\hat{H}'\ket{\phi_3}\]
        于是$\hat{H}'_{13}=\bra{\phi_1}\hat{H}'\ket{\phi_3}=0$.同样地,对于$i\in\{1,2,5\}$且$j\in\{3,4\}$(或者反之)都有$\hat{H}'_{ij}=0$.\\
        综上可得$\hat{H}'$是对角矩阵.
